...
class VoskNode(Node):
    def __init__(self):
        ...
        self.stream = sd.RawInputStream(samplerate=_SAMPLE_RATE, blocksize=_BLOCK_SIZE, dtype='int16', channels=1, callback=self._audio_callback)
        self.stream.start()
    ...
    def _evento_cb(self, msg: CAT793FEvents):
        ...
        try:
            evento_id = msg.events[0].code.data
            status = msg.events[0].status.data
            
            if status != "IN_PROGRESS":
                self.get_logger().info(f"Evento {evento_id} ignorado (Status não é IN_PROGRESS: {status}).")
                self.evento_ativo = {}
                return
            
            if not self._evento_permitido(evento_id):
                self.get_logger().warn(f"Evento {evento_id} ignorado — não permitido pelo sistema de IA.")
                self.evento_ativo = {}
                return
            ...
            self.get_logger().info(f"Evento ativo via GUI: [{evento_id}] Status: {status}")
            
        except Exception as e:
            self.get_logger().error(f'Erro ao processar mensagem do evento: {e}')
    ...
    def _publicar(self, texto: str):
        if not self.evento_ativo:
            self.get_logger().warn("Fala bloqueada: nenhum evento ativo no momento (ou status não é IN_PROGRESS).")
            return
            
        payload = {'fala'  : texto, 'evento': self.evento_ativo}
        m = String()
