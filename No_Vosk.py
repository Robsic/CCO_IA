#!/usr/bin/env python3
# Vosk → /fala_reconhecida (com evento ativo do supervisório)
...
class VoskNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('Nó Vosk iniciado. Aguardando acionamento do rádio...')

    def _evento_permitido(evento_id: int) -> bool:
            ...
            with self._vosk_lock:
                if self.rec.AcceptWaveform(data):
                    ...
                    if texto and not self.ja_publicou:
                        self.get_logger().info(f' Enviado: "{texto}"')
                        self._publicar(texto)
